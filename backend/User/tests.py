...
class UserListViewTests(TestCase):
	...
	def test_post_valid_data(self, mock_output_serializer, mock_input_serializer):
		...
		input_serializer_instance.validated_data = {'username': 'testuser', 'password': 'password123'}

		output_serializer_instance = mock_output_serializer.return_value
		output_serializer_instance.data = {'id': 1, 'username': 'testuser'}

		response = self.client.post(self.url, {'username': 'testuser', 'password': 'password123'}, format='json')
	...
	def test_post_invalid_data(self, mock_input_serializer):
		...
		input_serializer_instance.errors = {'username': ['This field is required.'], 'password': ['This field is required.']}
