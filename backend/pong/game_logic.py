...
class PongGame:
    def update_player_positions(pong_stat, game, key_press):
        ...
        if not pong_stat.game_started and key_press in ('o', 'l'):
            ...
            game.kick_start_game(pong_stat, key_press)
        if key_press == 'w':
    ...
    def kick_start_game(pong_stat, key_press):
    ...
    def update_ball_position(pong_stat, game):
        if pong_stat.game_started:
            ...
            game.check_collisions(pong_stat, game)  # Check for collisions

    def check_collisions(pong_stat, game):
        ...
        if pong_stat.ball_x <= pong_stat.player_left_x + PLAYER_WIDTH and pong_stat.player_left_y <= pong_stat.ball_y <= pong_stat.player_left_y + PLAYER_HEIGHT:
            game.handle_player_collision(pong_stat, game, pong_stat.player_left_y)
            ...
            game.handle_player_collision(pong_stat, game, pong_stat.player_right_y)
        ...
        if pong_stat.ball_x <= 0:
            game.handle_wall_collision(pong_stat, 'left')
        elif pong_stat.ball_x >= PLAYGROUND_WIDTH - BALL_WIDTH:
            game.handle_wall_collision(pong_stat, 'right')


    def handle_player_collision(pong_stat, game, player_y):
    ...
    def handle_wall_collision(pong_stat, wall):       
        if wall == 'left':
