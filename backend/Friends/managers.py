...
class FriendsManager(models.Manager):
	def get_user_friends(self, user_id: int, friendship_status: int) -> QuerySet[User]:
	...
	def get_friendship_by_user_and_friend_id(self, user_id: int, friend_id: int) -> 'Friend':
		return self.get(user_id=user_id, friend_id=friend_id)

	def get_friendship_by_id(self, friendship_id: int) -> 'Friend':
	...
	def delete_friendship(self, user_id: int, friend_id: int) -> None:
		user_friend = self.filter(user_id=user_id, friend_id=friend_id)
		friend_user = self.filter(user_id=friend_id, friend_id=user_id)
